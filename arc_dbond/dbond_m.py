...
class Encoder(nn.Module):
    def __init__(self,aa_type_count,pad_index,state_dim,env_dim,hidden_dim,num_heads, dropout, forward_expansion,attention_layer_num):
    ...
    def forward(self, seq_index_batch:torch.Tensor,
        ...
        # latent_vec_batch : [batch_len,1+state_dim+env_dim,HIDDEN_DIM]
        
        latent_vec_batch = torch.concat((seq_vec_batch,state_vec_batch,env_vec_batch),dim=1)
        # latent_vec_batch : [batch_len,(1+state_dim+env_dim)*HIDDEN_DIM]
...
class Decoder(nn.Module):
    def __init__(self,state_dim,env_dim,hidden_dim,output_dim, dropout,):
