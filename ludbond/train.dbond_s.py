...
validation_writer = SummaryWriter(tensorboard_log_pattern.format(model=MODEL,time=run_time,status = 'validation',tag = config['tag']))
...
for epoch in range(config['train_args']['epoch']):
    ...
    validation_metrics_dict = process(epoch=epoch,status='validation',writer=validation_writer,dataloader=validation_dataloader)
    ...
    if early_stop(validation_metrics_dict['AUC']):
        logging.info(f"{'#'*10} early stop {'#'*10}")
        logging.info(f"{'#'*10} epoch: [{epoch}/{config['train_args']['epoch']}] {'#'*10}")
        logging.info(f"{'#'*10} best validation AUC {best_validation_auc:.4} {'#'*10}")
        logging.info(f"{'#'*10} validation AUC {validation_metrics_dict['AUC']:.4} {'#'*10}")
        break
    if  validation_metrics_dict['AUC'] > best_validation_auc:
        best_validation_auc = validation_metrics_dict['AUC']
