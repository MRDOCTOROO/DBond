"""DBond-m-pre 5fold 训练(R-01 pre 变体, 对齐 DBond-GT-pre)。

与 train_dbond_m_5fold.py 的唯一差异: 默认配置指向 dbond_m_config/pre.yaml
(zero_fill intensity/scan_num), 结果写到 result/cv/dbond_m_pre/, 不与 dbond_m 混。

用法(云端):
  python ludbond/train_dbond_m_pre_5fold.py --config ludbond/dbond_m_config/pre.yaml \
      --fold_data_dir dataset/5fold
  python ludbond/train_dbond_m_pre_5fold.py --folds 1222   # 调试单 fold

结果输出:
  ./result/cv/dbond_m_pre/{timestamp}/fold_{id}/{best_model,metric,pred,...}/
  ./result/cv/dbond_m_pre/{timestamp}/5fold_metrics.csv  (每 fold 一行)
  ./result/cv/dbond_m_pre/{timestamp}/5fold_summary.csv  (mean ± std)
"""
