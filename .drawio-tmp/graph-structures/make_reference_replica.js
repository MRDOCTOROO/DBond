...
const OUT = 'D:/UserData/Desktop/graph-structures-reference-replica.drawio';
const W = 1720, H = 1080;
let cells = [];
let nextId = 2;
...
function node(label, x, y, r = 27, fill = '#f8fbff', stroke = '#23466d') {
  const style = `ellipse;whiteSpace=wrap;html=1;aspect=fixed;fillColor=${fill};strokeColor=${stroke};strokeWidth=2;fontFamily=Times New Roman;fontSize=24;fontStyle=2;fontColor=#111111;align=center;verticalAlign=middle;`;
  return cell(label, style, x - r, y - r, 2 * r, 2 * r);
}
...
function seqEdges(xs, y, color = '#3366cc') {
  [[0,1], [1,2], [2,3], [4,5]].forEach(([i,j]) => line(xs[i] + 27, y, xs[j] - 27, y, color, 3));
}
function arc(x1, x2, y, height, color, dashed = false, dash = '8 6') {
  const mid = (x1 + x2) / 2;
  line(x1, y - 24, x2, y - 24, color, 2.2, dashed, dash, false, [[mid, y - height]]);
}
function bottomArc(x1, x2, y, depth, color, dashed = true, dash = '12 6 4 6') {
  const mid = (x1 + x2) / 2;
  line(x1, y + 24, x2, y + 24, color, 2.2, dashed, dash, false, [[mid, y + depth]]);
}
...
graphNodes(xsb, ytop, labels); seqEdges(xsb, ytop, stroke);
[[620,702,75],[702,784,75],[784,866,75],[620,784,90],[702,866,90],[866,1010,75]].forEach(a => arc(...a, green));
[[620,866,132],[702,1010,150],[784,1090,128],[866,1090,75]].forEach(a => arc(...a, orange, true, '8 6'));
...
graphNodes(xsc, ytop, labels); seqEdges(xsc, ytop, stroke);
[[1188,1270,75],[1270,1352,75],[1352,1434,75],[1188,1352,90],[1270,1434,90],[1434,1584,75],[1584,1670,75]].forEach(a => arc(...a, green));
[[1188,1434,132],[1270,1584,150],[1352,1670,128],[1434,1670,75]].forEach(a => arc(...a, orange, true, '8 6'));
[[1188,1584],[1270,1670]].forEach(([a,b]) => { arc(a, b, ytop, 98, purple, true, '12 6 4 6'); bottomArc(a, b, ytop, 100, purple); });
...
const xml = `<mxfile host="app.diagrams.net" modified="2026-07-14T00:00:00.000Z" agent="ZCode" version="24.7.17" type="device"><diagram id="graph-ref" name="Page-1"><mxGraphModel dx="1720" dy="1080" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="${W}" pageHeight="${H}" math="1" shadow="0"><root><mxCell id="0"/><mxCell id="1" parent="0"/>${cells.join('')}</root></mxGraphModel></diagram></mxfile>`;
