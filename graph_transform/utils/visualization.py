...
def plot_single_sample_layer_attention(
    ...
    """
    绘制单个样本各层注意力权重分布（用于论文案例分析）
    
    Args:
        attention_weights_list: 各层注意力权重列表 [layer0, layer1, ...]
        bond_labels: 键断裂标签 [num_bonds]
        sequence: 氨基酸序列
        edge_index: 边索引
        save_path: 保存路径
        figsize: 图形大小
        max_seq_len: 最大显示序列长度
    ...
    # 提取各层的肽键级别注意力
    all_bond_attn = []
    for layer_idx, layer_weights in enumerate(attention_weights_list):
        bond_attn, bond_labels_str = extract_bond_level_attention(
            layer_weights, edge_index, sequence, max_seq_len
        )
        all_bond_attn.append(bond_attn)
    ...
    plt.subplots_adjust(wspace=0.15, left=0.08, right=0.95, top=0.88, bottom=0.15)
    ...
    for layer_idx, (ax, bond_attn) in enumerate(zip(axes, all_bond_attn)):
        # 绘制背景（断裂键位置）
        for i in range(num_bonds):
            if i < len(bond_labels_np) and bond_labels_np[i] == 1:
                ax.axvspan(i - 0.4, i + 0.4, alpha=0.15, color=colors['broken'])
        
        # 绘制注意力权重柱状图
        bar_colors = [colors['broken'] if bond_labels_np[i] == 1 else colors['intact'] 
                     for i in range(num_bonds)]
        ax.bar(x, bond_attn[:num_bonds], color=bar_colors, alpha=0.7, edgecolor='white')
        
        # 标记Top-3
        top_k = min(3, num_bonds)
        top_indices = np.argsort(bond_attn[:num_bonds])[-top_k:]
        for idx in top_indices:
            ax.annotate(bond_labels_str[idx], 
                       xy=(idx, bond_attn[idx]),
                       xytext=(0, 8), textcoords='offset points',
                       ha='center', fontsize=7, fontweight='bold',
                       color=colors['highlight'])
        
        # 计算该层的相关系数
        if len(bond_attn[:num_bonds]) == len(bond_labels_np):
            corr = np.corrcoef(bond_attn[:num_bonds], bond_labels_np)[0, 1]
        else:
            corr = 0.0
    ...
    # 总标题
    seq_display = sequence[:20] + ('...' if len(sequence) > 20 else '')
    fig.suptitle(f'Sample Peptide: {seq_display}', fontsize=12, fontweight='bold', y=0.98)
    
    # 添加图例
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor=colors['broken'], alpha=0.7, label='Broken Bond'),
        Patch(facecolor=colors['intact'], alpha=0.7, label='Intact Bond')
    ]
    fig.legend(handles=legend_elements, loc='lower center', ncol=2, fontsize=9, 
              bbox_to_anchor=(0.5, 0.02))
    ...
    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Saved single sample layer attention to {save_path}")
    
    return fig


def plot_layer_evolution_trend(
    layer_correlations: List[float],
    layer_names: Optional[List[str]] = None,
    save_path: Optional[str] = None,
    figsize: Tuple[int, int] = (8, 5)
) -> plt.Figure:
    """
    绘制层间演化趋势图（注意力-断裂相关性随层数变化）
    
    Args:
        layer_correlations: 各层的相关系数列表 [r_layer0, r_layer1, ...]
        layer_names: 各层名称（可选）
        save_path: 保存路径
        figsize: 图形大小
    ...
    num_layers = len(layer_correlations)
    x = np.arange(num_layers)
    ...
    # 绘制折线图
    ax.plot(x, layer_correlations, 'o-', color='#2C3E50', linewidth=2.5, 
            markersize=10, markerfacecolor='#E74C3C', markeredgecolor='black', 
            markeredgewidth=1.5, zorder=3)
    
    # 添加数值标签
    for i, (xi, yi) in enumerate(zip(x, layer_correlations)):
        ax.annotate(f'{yi:.3f}', xy=(xi, yi), xytext=(0, 12), 
                   textcoords='offset points', ha='center', fontsize=10, 
                   fontweight='bold', color='#2C3E50')
    
    # 填充区域
    ax.fill_between(x, layer_correlations, alpha=0.2, color='#3498DB')
    
    # 添加趋势线
    if num_layers > 2:
        z = np.polyfit(x, layer_correlations, 1)
        p = np.poly1d(z)
        ax.plot(x, p(x), '--', color='#E74C3C', alpha=0.5, linewidth=1.5, 
                label=f'Trend (slope={z[0]:.4f})')
        ax.legend(fontsize=10)
    
    ax.set_xlabel('Network Layer', fontsize=12, fontweight='bold')
    ax.set_ylabel('Attention-Breakage Correlation (r)', fontsize=12, fontweight='bold')
    ax.set_title('Layer-wise Evolution of Attention Correlation', 
                fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(layer_names, fontsize=10)
    ax.set_ylim(min(layer_correlations) - 0.1, max(layer_correlations) + 0.15)
    ax.grid(True, alpha=0.3)
    
    # 添加解释性文字
    if layer_correlations[-1] > layer_correlations[0]:
        ax.text(0.02, 0.98, 'Correlation increases with depth\n→ Model refines bond prediction', 
    ...
    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Saved layer evolution trend to {save_path}")
...
def plot_bond_type_comparison(
    ...
    """
    绘制断裂键vs完整键注意力权重对比（含统计检验）
    
    Args:
        attention_weights_list: 各样本的注意力权重列表
        bond_labels_list: 各样本的键断裂标签列表
        edge_indices: 各样本的边索引列表
        sequences: 各样本的序列列表
        save_path: 保存路径
        figsize: 图形大小
    ...
    for attn_weights, labels, edge_idx, seq in zip(
        ...
        bond_attn, _ = extract_bond_level_attention(last_layer, edge_idx, seq)
        labels_np = labels.cpu().numpy() if isinstance(labels, torch.Tensor) else labels
        
        num_bonds = min(len(bond_attn), len(labels_np))
        for i in range(num_bonds):
            if labels_np[i] == 1:
                broken_weights.append(bond_attn[i])
            else:
                intact_weights.append(bond_attn[i])
    ...
    # 添加散点
    np.random.seed(42)
    x1 = np.random.normal(1, 0.06, size=len(broken_weights))
    x2 = np.random.normal(2, 0.06, size=len(intact_weights))
    ax.scatter(x1, broken_weights, alpha=0.4, color='#E74C3C', s=20, zorder=2)
    ax.scatter(x2, intact_weights, alpha=0.4, color='#3498DB', s=20, zorder=2)
    
    # 统计检验
    t_stat, p_value = stats.ttest_ind(broken_weights, intact_weights)
    
    # 计算效应量 (Cohen's d)
    pooled_std = np.sqrt((np.std(broken_weights)**2 + np.std(intact_weights)**2) / 2)
    cohens_d = (np.mean(broken_weights) - np.mean(intact_weights)) / pooled_std if pooled_std > 0 else 0
    
    # 添加统计结果文本框
    stats_text = (f't = {t_stat:.2f}\n'
                 f'p = {p_value:.4f}\n'
                 f"Cohen's d = {cohens_d:.2f}\n"
                 f'n_broken = {len(broken_weights)}\n'
                 f'n_intact = {len(intact_weights)}')
    ...
    ax.set_ylabel('Attention Weight', fontsize=12, fontweight='bold')
    ax.set_title('Attention Weight Distribution: Broken vs Intact Bonds', 
    ...
    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Saved bond type comparison to {save_path}")
...
def compute_layer_correlations(
    ...
    Args:
        attention_weights_list: 各样本的各层注意力权重 [[layer0, layer1, ...], ...]
        bond_labels_list: 各样本的键断裂标签
        edge_indices: 各样本的边索引
        sequences: 各样本的序列
    ...
    num_layers = len(attention_weights_list[0])
    layer_correlations = [[] for _ in range(num_layers)]
    ...
    for attn_weights, labels, edge_idx, seq in zip(
        ...
        for layer_idx in range(min(num_layers, len(attn_weights))):
            bond_attn, _ = extract_bond_level_attention(attn_weights[layer_idx], edge_idx, seq)
            
            num_bonds = min(len(bond_attn), len(labels_np))
            if num_bonds > 2:
                corr = np.corrcoef(bond_attn[:num_bonds], labels_np[:num_bonds])[0, 1]
                if not np.isnan(corr):
                    layer_correlations[layer_idx].append(corr)
    
    # 计算各层的平均相关系数
    avg_correlations = []
    for layer_corrs in layer_correlations:
        if layer_corrs:
            avg_correlations.append(np.mean(layer_corrs))
        else:
...
def plot_new_interpretability_case_study(
    ...
    """
    绘制新版4子图可解释性案例分析图（优化版）
    
    布局:
    (a) 单样本各层注意力分布 - 展示模型如何逐层学习
    (b) 层间演化趋势 - 展示相关性随层数变化
    (c) 断裂vs完整键注意力对比 - 展示模型区分能力
    (d) 注意力分布热力图 - 展示整体模式
    
    Args:
        attention_weights_list: 各样本的各层注意力权重列表
        bond_labels_list: 各样本的键断裂标签列表
        sequences: 各样本的序列列表
        edge_indices: 各样本的边索引列表
        save_path: 保存路径
    ...
    for layer_idx in range(num_layers):
        bond_attn, bond_labels_str = extract_bond_level_attention(
            sample_attn[layer_idx], sample_edge_idx, sample_seq, max_seq_len
        )
        layer_bond_attn.append(bond_attn)
    
    bond_labels_np = sample_labels[:num_bonds].cpu().numpy() if isinstance(sample_labels, torch.Tensor) else sample_labels[:num_bonds]
    
    # 绘制最后一层的注意力分布
    last_layer_attn = layer_bond_attn[-1][:num_bonds]
    x = np.arange(num_bonds)
    
    # 背景色
    for i in range(num_bonds):
        if i < len(bond_labels_np) and bond_labels_np[i] == 1:
            ax1.axvspan(i - 0.4, i + 0.4, alpha=0.15, color=colors['broken'])
    
    # 柱状图
    bar_colors = [colors['broken'] if bond_labels_np[i] == 1 else colors['intact'] 
                 for i in range(num_bonds)]
    ax1.bar(x, last_layer_attn, color=bar_colors, alpha=0.7, edgecolor='white')
    
    # Top-3标注
    top_indices = np.argsort(last_layer_attn)[-3:]
    for idx in top_indices:
        ax1.annotate(bond_labels_str[idx], 
                    xy=(idx, last_layer_attn[idx]),
                    xytext=(0, 8), textcoords='offset points',
                    ha='center', fontsize=7, fontweight='bold',
                    color=colors['highlight'])
    
    # 计算相关系数
    corr = np.corrcoef(last_layer_attn, bond_labels_np[:num_bonds])[0, 1]
    ...
    if layer_corrs:
        layer_x = np.arange(len(layer_corrs))
        ax2.plot(layer_x, layer_corrs, 'o-', color=colors['line'], linewidth=2.5,
                markersize=10, markerfacecolor=colors['broken'], markeredgecolor='black',
                markeredgewidth=1.5, zorder=3)
        
        # 数值标签
        for xi, yi in zip(layer_x, layer_corrs):
            ax2.annotate(f'{yi:.3f}', xy=(xi, yi), xytext=(0, 12),
                        textcoords='offset points', ha='center', fontsize=10,
                        fontweight='bold', color=colors['line'])
        
        # 填充
        ax2.fill_between(layer_x, layer_corrs, alpha=0.2, color=colors['intact'])
        ...
        if len(layer_corrs) > 2:
            ...
            # 解释文字
            if z[0] > 0:
                ax2.text(0.02, 0.98, 'Correlation increases\n→ Model refines prediction', 
                        transform=ax2.transAxes, fontsize=9, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
        
        ax2.set_xticks(layer_x)
        ax2.set_xticklabels([f'L{i}' for i in range(len(layer_corrs))], fontsize=10)
    
    ax2.set_xlabel('Network Layer', fontsize=10, fontweight='bold')
    ax2.set_ylabel('Correlation Coefficient (r)', fontsize=10, fontweight='bold')
    ax2.set_title('(b) Layer-wise Evolution Trend', fontsize=11, fontweight='bold')
    ax2.grid(True, alpha=0.3)
    ...
    x1 = np.random.normal(1, 0.06, size=len(broken_weights))
    x2 = np.random.normal(2, 0.06, size=len(intact_weights))
    ax3.scatter(x1, broken_weights, alpha=0.4, color=colors['broken'], s=20, zorder=2)
    ax3.scatter(x2, intact_weights, alpha=0.4, color=colors['intact'], s=20, zorder=2)
    ...
    ax3.set_ylabel('Attention Weight', fontsize=10, fontweight='bold')
    ax3.set_title('(c) Broken vs Intact Bond Attention', fontsize=11, fontweight='bold')
    ax3.grid(True, alpha=0.3, axis='y')
    ...
    for layer_idx in range(num_layers):
        ...
        for attn_weights, edge_idx, seq in zip(
            ...
            # 归一化到[0,1]
            if bond_attn.max() > 0:
                bond_attn = bond_attn / bond_attn.max()
            layer_attn_all.append(bond_attn[:15])  # 取前15个位置
        
        # 平均
        max_len = max(len(a) for a in layer_attn_all)
        padded = [np.pad(a, (0, max_len - len(a)), constant_values=0) for a in layer_attn_all]
        heatmap_data.append(np.mean(padded, axis=0))
    
    heatmap_array = np.array(heatmap_data)
    
    im = ax4.imshow(heatmap_array, cmap='YlOrRd', aspect='auto', interpolation='nearest')
    plt.colorbar(im, ax=ax4, label='Normalized Attention')
    
    ax4.set_xlabel('Bond Position', fontsize=10, fontweight='bold')
    ax4.set_ylabel('Network Layer', fontsize=10, fontweight='bold')
    ax4.set_title('(d) Attention Heatmap (All Samples)', fontsize=11, fontweight='bold')
    ...
    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Saved new interpretability case study to {save_path}")
