...
def create_datasets(config: Dict[str, Any]) -> tuple:
    ...
    # 打印数据集统计信息
    logger.info("Dataset creation time: %.4fs", time.perf_counter() - dataset_start)
...
def create_data_loaders(train_dataset, val_dataset, test_dataset, 
    ...
    logger.info("Dataloader creation time: %.4fs", time.perf_counter() - dataloader_start)
    logger.info(
        "Dataloader params - batch_size: %s, num_workers: %s, pin_memory: %s, drop_last: %s",
...
def main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        logger.info("Epoch %s wall time: %.4fs", epoch + 1, time.perf_counter() - epoch_start)
    ...
    logger.info("Training completed in %.4fs", time.perf_counter() - overall_start)
