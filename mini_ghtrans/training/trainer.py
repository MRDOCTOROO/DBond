...
class Trainer:
    ...
    def __init__(self,
        ...
        self.profile_time = self.config.get('debug', {}).get('profile_time', False)
    ...
    def train_epoch(self, train_loader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            if batch_idx % self.training_config.get('log_interval', 10) == 0:
                if self.profile_time:
                    self.logger.info(
                        "Epoch %s Batch %s Timing - fetch_wait: %.4fs, move: %.4fs, forward: %.4fs, backward+opt: %.4fs, total: %.4fs, loss: %.4f",
                        epoch,
                        batch_idx,
                        fetch_wait_time,
                        move_time,
        ...
        # 记录epoch总结
        self.logger.info(
            "Epoch %s Training - Loss: %.4f, AvgBatch: %.4fs, Move: %.4fs, Forward: %.4fs, Backward: %.4fs",
            epoch,
            avg_loss,
