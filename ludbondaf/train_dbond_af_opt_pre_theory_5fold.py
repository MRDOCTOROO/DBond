"""DBond-AF-opt-pre+theory 5fold 训练(R-01 pre 变体, 对齐 DBond-GT-pre)。

与 train_dbond_af_opt_5fold.py 的唯一差异: 默认配置指向 af_opt_pre_theory.yaml
(zero_fill intensity/scan_num; hidden=64/dropout=0.4 等 AF-opt 既有超参不变),
结果写到 result/cv/dbond_af_opt_pre_theory/, 不与 dbond_af_opt 混。

用法(云端, 在仓库根目录运行):
  python ludbondaf/train_dbond_af_opt_pre_5fold.py --config ludbondaf/dbond_m_exp_af_config/af_opt_pre_theory.yaml \
      --fold_data_dir dataset/5fold
  python ludbondaf/train_dbond_af_opt_pre_5fold.py --folds 1222   # 调试单 fold

结果输出:
  ./result/cv/dbond_af_opt_pre_theory/{timestamp}/fold_{id}/{best_model,metric,pred,...}/
  ./result/cv/dbond_af_opt_pre_theory/{timestamp}/5fold_metrics.csv  (每 fold 一行)
  ./result/cv/dbond_af_opt_pre_theory/{timestamp}/5fold_summary.csv  (mean ± std)
"""
import os
import sys
...
MODEL_NAME = 'dbond_af_opt_pre_theory'
# 默认配置相对本脚本目录解析(CWD 无关; run_5fold 按 CWD 打开相对路径会踩坑)
DEFAULT_CONFIG = os.path.join(_THIS_DIR, 'dbond_m_exp_af_config', 'af_opt_pre_theory.yaml')
