...
class PepDataset(Dataset):
    def __init__(self,config:str,split:str='train') -> None:
        super().__init__()
        ...
        if split not in split_path_key:
            raise ValueError(f'unsupported dataset split: {split}')
        csv_key = split_path_key[split]
        if csv_key not in config['csv']:
            raise KeyError(f'missing csv config for split {split}: {csv_key}')

        self.csv_path = config['csv'][csv_key]
        if not os.path.exists(self.csv_path):
            raise FileNotFoundError(
                f'{split} dataset file does not exist: {self.csv_path}'
            )
