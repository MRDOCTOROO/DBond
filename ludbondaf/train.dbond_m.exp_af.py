"""DBond-AF (attention-based feature fusion) 训练脚本 — 对齐 DBond-GT 协议版。

本脚本在 DBond-GT 完全相同的数据划分与评价协议下复现 DBond-AF 基线,
解决审稿 R-05/R-06 的公平性硬伤。协议对齐点:
  - 模型选择: 按 val_f1 选 best model(不再用 val_loss), 与 DBond-GT / DBond-m 一致
  - val_f1 含 bond mask(屏蔽 padding), 与 DBond-GT valid-key 展平 F1 同口径
  - val 切分: 从 train 内 random_split 20%(seed=42+fold_index), 不再读外部 val CSV
  - early_stopping: 严格提升(> best+delta)才更新 best, 与 DBond-GT 语义一致
  - 训练结束重载 best model 在 test 上评估, 输出 test_metric.csv + test.pred.csv
训练超参(bs/ep/optimizer/loss)保留 DBond-AF 原始设置, 仅统一协议层。
"""
import torch
import yaml
...
# 用 dbond_af 作为 5fold 聚合标识, 避免与 dbond_m 的产物目录混淆
MODEL = "dbond_af"
...
def build_loss_func(config) -> Callable:
    # DBond-AF 原始 loss: multilabel_soft_margin_loss(保留原始, 不改)
    if config["train_args"]["loss_type"].lower() == "ce":
        ...
        from dbond_m_exp_af import multilabel_categorical_crossentropy
...
def _run_epoch(epoch, status, writer, dataloader, dataset_len, model, loss_func,
    ...
    """跑一个 epoch 的 train 或 validation(模块级纯函数, 显式传参)。
    optimizer 非 None 时反传训练, None 时仅前向(eval)。
    保留 DBond-AF 原始的梯度直方图日志(训练模式, 每 50 batch 记一次)。
    """
    ...
    metrics_dict = {
        ...
        "lab_acc_ma": multi_label_metrics.label_accuracy_macro(gt, predict, mask=mask),
        "lab_acc_mi": multi_label_metrics.label_accuracy_micro(gt, predict, mask=mask),
        "lab_precision_ma": multi_label_metrics.label_precision_macro(gt, predict, mask=mask),
        "lab_precision_mi": multi_label_metrics.label_precision_micro(gt, predict, mask=mask),
        "lab_recall_ma": multi_label_metrics.label_recall_macro(gt, predict, mask=mask),
        "lab_recall_mi": multi_label_metrics.label_recall_micro(gt, predict, mask=mask),
        "lab_f1_ma": multi_label_metrics.label_f1_macro(gt, predict, mask=mask),
        "lab_f1_mi": multi_label_metrics.label_f1_micro(gt, predict, mask=mask),
...
def main(config: dict, run_id: str = None) -> Dict:
    ...
    run_id: 可选运行标识(如 fold_1222)。None 时用时间戳。
    """
    # ===== seed =====
    ...
    tensorboard_dir = config.get("output", {}).get(
        "tensorboard_dir",
        tensorboard_log_pattern.format(model=MODEL, time=run_id, status="run", tag=config["tag"]),
    )
    ...
    logging.info(
        f"train_size={train_size}, val_size={val_size} (random_split from train, seed={seed})"
    )
    ...
    train_writer = SummaryWriter(tensorboard_dir + "__train")
    validation_writer = SummaryWriter(tensorboard_dir + "__validation")
    ...
    for epoch in range(config["train_args"]["epoch"]):
        train_metrics_dict = _run_epoch(
            epoch, "train", train_writer, train_dataloader, len(train_dataset),
            model, loss_func, device, config, optimizer,
        )
        validation_metrics_dict = _run_epoch(
            epoch, "validation", validation_writer, validation_dataloader, len(validation_dataset),
            model, loss_func, device, config, None,
        )
        logging.info(
            f"{'#'*10} validation val_f1 {validation_metrics_dict['val_f1']:.4} {'#'*10}"
        )
    ...
    test_metrics = _evaluate_on_test(
        model, best_validation_model_path, test_dataloader, loss_func, device,
        len(test_dataset), config, result_metric_dir, result_pred_dir, run_id,
    )
...
def evaluate_only(config: dict, run_id: str = None) -> Dict:
    ...
    import glob
    candidates = sorted(
        glob.glob(os.path.join(best_model_dir, "best_model_*.pt")),
    ...
    test_metrics = _evaluate_on_test(
        model, best_model_path, test_dataloader, loss_func, device,
        len(test_dataset), config, result_metric_dir, result_pred_dir, run_id,
    )
...
def _evaluate_on_test(model, best_model_path, test_dataloader, loss_func, device,
    ...
    val_f1 含 bond mask, 与 DBond-GT valid-key 展平 F1 同口径。"""
    ...
    # 输出 pred csv(多标签展平: true/pred/pred_prob, 对齐 GT latest_test.pred.csv 风格)
